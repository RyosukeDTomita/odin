...
class CookieCheckerTest {
  ...
  void https_withSecure_noMissingSecureFinding() {
    List<Finding> f =
        check("https://example.com", "s=1; HttpOnly; Secure; SameSite=Lax; Max-Age=3600");
    assertNotContains(f, IssueDefinition.COOKIE_MISSING_SECURE);
  }
  ...
  void withDomain_reportsDomainAttribute() {
    List<Finding> f =
        check("http://example.com", "s=1; HttpOnly; SameSite=Lax; Domain=example.com");
  ...
  void sameSiteNoneWithSecure_reportsSameSiteNone() {
    List<Finding> f =
        check("https://example.com", "s=1; HttpOnly; Secure; SameSite=None; Max-Age=3600");
